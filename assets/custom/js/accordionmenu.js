...
function McAcdnMenu(s) {
    var k = function (a, b) {
    ...
    }, h = "className", N = 0, v = "firstChild", j = function (b, c) {
        var a = c == 0 ? b.nextSibling : b[v];
        while (a && a.nodeType != 1)
            a = a.nextSibling;
        return a
    }, a = "length", u = "attachEvent", y = "addEventListener", n = function (e) {
        var b = e.childNodes, d = [];
        if (b)
            for (var c = 0, f = b[a]; c < f; c++)
                b[c].nodeType == 1 && d.push(b[c]);
        return d
    }, o = "nodeName", cb = function (c) {
        var b = [], d = c[a];
        while (d--)
            b.push(String.fromCharCode(c[d]));
        return b.join("")
    }, b = "parentNode", d = "style", X = function (b, d) {
        var c = b[a];
        while (c--)
            if (b[c] === d)
                return true;
        return false
    }, c = "offsetHeight", r = "insertBefore", l = function (b, a) {
        return X(b[h].split(" "), a)
    }, D = "setAttribute", p = function (a, b, c) {
        if (!l(a, b))
            if (a[h] == "")
                a[h] = b;
            else if (c)
                ...
                a[h] += " " + b
    }, i = "replace", f = "height", U = function (a, b) {
        var c = new RegExp("(^| )" + b + "( |$)");
        a[h] = a[h][i](c, "$1");
        a[h] = a[h][i](/ $/, "")
    }, t = null, m, e, M = document, q = "createElement", A = "getElementById", bb = ["$1$2$3", "$1$2$3", "$1$24", "$1$23", "$1$22"], z, H, ab = [/(?:.*\.)?(\w)([\w\-])[^.]*(\w)\.[^.]+$/, /.*([\w\-])\.(\w)(\w)\.[^.]+$/, /^(?:.*\.)?(\w)(\w)\.[^.]+$/, /.*([\w\-])([\w\-])\.com\.[^.]+$/, /^(\w)[^.]*(\w)$/], O = function (a) {
        return a[i](/(?:.*\.)?(\w)([\w\-])?[^.]*(\w)\.[^.]*$/, "$1$3$2")
    }, x = function (e, b, f) {
        var d = [];
        if (f && ((new Date).getTime() - 500 > H || N))
            return 1;
        for (var c = 0; c < e[a]; c++)
        ...
        var e = function (b) {
            for (var d = b.substr(0, b[a] - 1), f = b.substr(b[a] - 1, 1), e = "", c = 0; c < d[a]; c++)
                e += d.charCodeAt(c) - f;
            return unescape(e)
        }, b = O(document.domain) + Math.random(), c = e(b);
        ...
        m = {a: s.license || "5432", b: s.menuId, c: s.linkIdToMenuHtml, e: s.expand, g: s.linkIdToMenuHtml}
    }, S = function (n) {
        for (var f = -1, h = -1, j = g("location").href.toLowerCase()[i]("www.", "")[i](/([\-\[\].$()*+?])/g, "\\$1") + "$", l = new RegExp(j, "i"), d, e = k(n, x("d", 0, true)), c = 0; c < e[a]; c++)
            if (e[c].href) {
    ...
    I.prototype = {c: function (c) {
            ...
            this.l(d, 1);
            var s = this.H && l(d[b][b], "top") ? this.H : this.m(r), f = null;
            if (i) {
                f = n(d[b][b]);
                for (var g = 0; g < f[a]; g++)
                    if (f[g][o] == "LI") {
            ...
            if (t)
                for (var v = B(j.parentNode), q = n(this.b), r, m = 0; m < q[a]; m++)
                    if (q[m] != v) {
                        r = k(q[m], "ul");
                        if (r[a]) {
                            b = r[0];
    ...
    var P = function (c) {
        ...
        a.onreadystatechange = function () {
            if (a.readyState == 4 && a.status == 200) {
                var e = a.responseText, h = /^[\s\S]*<body[^>]*>([\s\S]+)<\/body>[\s\S]*$/i;
                if (h.test(e))
                    e = e[i](h, "$1");
                e = e[i](/^\s+|\s+$/g, "");
                var f = g(q, "div");
        ...
        if (g("addEventListener"))
            document[y]("DOMContentLoaded", a, false);
        else if (g(u)) {
            try {
        ...
        if (window[y])
            window[y]("load", a, false);
        else
            window[u] && window[u]("onload", a)
    };
    Q();
    var Z = g(q, "nav"), L = k(document, "head");
    ...
    return{init: F, open: function (L_li_id, L_closeOthers) {
